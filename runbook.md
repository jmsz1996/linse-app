...
`linse_uploads` (photos). Back up both.
...
`linse_pgdata` and `linse_uploads` — database and photos gone. Use plain
`docker compose down` to stop. Only use `-v` when you intend to wipe everything.
